...
try {
  ...
  assert.match(workerScript, /@version\s+2\.2\.3/);
  assert.match(workerScript, /const APP_VERSION = "2\.2\.3"/);
  ...
  assert.match(workerScript, /页面操作可见 · 请勿手动操作/);
  ...
  assert.match(indeedWorkerScript, /@version\s+2\.2\.4/);
  assert.match(indeedWorkerScript, /const APP_VERSION = "2\.2\.4"/);
  ...
  assert.match(indeedWorkerScript, /页面操作可见 · 请勿手动操作/);
  ...
  assert.match(linkedInWorkerScript, /@version\s+2\.2\.3/);
  assert.match(linkedInWorkerScript, /const APP_VERSION = "2\.2\.3"/);
  ...
  assert.match(linkedInWorkerScript, /页面操作可见 · 请勿手动操作/);
  ...
  assert.match(dashboardScript, /name: "Indeed", version: "v2\.2\.4"/);
  assert.match(dashboardScript, /\/workers\/install\//);
  assert.match(dashboardScript, /agent-worker-/);
  const installerResponse = await fetch("http://127.0.0.1:" + port + "/workers/install/indeed-agent-worker-v2.2.4.user.js");
  ...
  assert.match(installerScript, /@version\s+2\.2\.4/);
