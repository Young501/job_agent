...
flowchart TD
    ...
    B --> B1["启动前选择画像与运行方式<br/>AI 审阅或仅采集职位"]
...
5. 预检成功后点击“开始运行”，先选择本次使用的画像，并在“启用 AI 审阅”和“仅采集职位”之间作出选择，再确认该画像的排除词。启用 AI 时会自动获取候选职位 JD 并评分；仅采集时仍会执行统一去重和本地标题筛查，但不会获取 JD 或调用 AI，之后仍可对单个职位手动发起 AI 审阅。运行创建后会保存运行方式、画像、学习偏好和排除词快照，后续编辑设置不会改变已经开始的这轮处理依据。三个平台严格按顺序执行，不会并行操作同一浏览器。
6. Worker 扫描到职位卡片后，先把当页候选交给 Agent。Agent 立即用统一历史和本地规则返回“跳过、复用、排除、仅采集或获取 JD”的计划，因此无需先把整份历史复制给脚本。只有本次启用了 AI 审阅的不确定职位才继续进入 JD 流程。
